...
/** Server-side fetch against the public (unauthenticated) API surface. Revalidates every 60s (ISR-style). */
export async function publicFetch<T>(path: string): Promise<T> {
  const res = await fetch(`${API_BASE_URL}${path}`, { headers: getTenantHeader(), next: { revalidate: 60 } });
